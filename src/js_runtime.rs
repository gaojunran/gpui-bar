...
/// 从当前 ctx 的 pending exception 中提取 message + stack,写入统一日志。
/// rquickjs 的 `Error::Exception` 会吞掉真实 message(只给 "Exception generated by QuickJS"),
/// 必须用 `Ctx::catch()` 拿到 JS 侧的 Error 对象再读 `.message` / `.stack`。
/// 无 pending exception 时什么都不做。
fn log_pending_exception<'js>(ctx: &Ctx<'js>, tag: &str) {
    if !ctx.has_exception() {
        return;
    }
    let exc = ctx.catch();
    if let Some(obj) = exc.clone().into_object() {
        if let Some(ex) = rquickjs::Exception::from_object(obj) {
            let msg = ex.message().unwrap_or_default();
            let stack = ex.stack().unwrap_or_default();
            write_log(tag, &format!("JS exception: {msg}\nstack: {stack}"));
            return;
        }
        let _ = ctx.globals().set("__exc_val", exc);
        let s: String = ctx
            .eval::<String, _>("JSON.stringify(__exc_val)")
            .unwrap_or_else(|_| "<non-serializable exception>".into());
        write_log(tag, &format!("JS exception (non-Error): {s}"));
    } else {
        let s: String = exc
            .as_string()
            .and_then(|v| v.to_string().ok())
            .unwrap_or_else(|| "<non-string, non-object exception>".into());
        write_log(tag, &format!("JS exception (primitive): {s}"));
    }
}
...
pub fn run_config(config_path: &Path) -> anyhow::Result<Value> {
    ...
    let value = ctx.with(|ctx| -> anyhow::Result<Value> {
        ...
        eval_promise.finish::<()>().inspect_err(|_| {
            log_pending_exception(&ctx, "[config]");
            write_log("[config]", "module.eval() rejected");
        })?;

        let config_func: Function = evaluated.get("default").map_err(|e| {
            log_pending_exception(&ctx, "[config]");
            anyhow::anyhow!("get default export failed: {e:?}")
        })?;

        // 调用 async getConfig，finish 驱动 job queue 完成
        let call_promise: Promise = config_func.call(())?;
        let raw: rquickjs::Value = call_promise.finish().inspect_err(|_| {
            log_pending_exception(&ctx, "[config]");
            write_log("[config]", "getConfig() promise rejected");
        })?;

        // 用 JSON.stringify 桥接 JS Value -> serde_json::Value
        ctx.globals().set("__config_result", raw)?;
        let json_str: String = ctx.eval::<String, _>("JSON.stringify(__config_result)").inspect_err(|_| {
            log_pending_exception(&ctx, "[config]");
            write_log("[config]", "JSON.stringify(__config_result) threw");
        })?;
